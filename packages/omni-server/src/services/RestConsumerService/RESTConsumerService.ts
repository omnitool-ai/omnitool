    ...
    if (payload.integration.key.startsWith('huggingface')) {
      ...
      switch (endpoint) {
        case 'text_to_image': {
          ...
          //@ts-ignore
          const image_cdn = await this.app.cdn.putTemp(buffer, {
            mimeType: type,
        ...
        case 'image_to_text': {
          ...
          for (const image_cdn of image_cdns) {
            //@ts-ignore
            const raw_image = await this.app.cdn.get(image_cdn.ticket);
        ...
        case 'image_to_image': {
          ...
          for (const image_cdn of image_cdns) {
            ...
            //@ts-ignore
            const result_cdn = await this.app.cdn.putTemp(buffer, {
              mimeType: type,
              userId: payload.job_ctx?.userId,
              jobId: payload.job_ctx?.jobId
            });
